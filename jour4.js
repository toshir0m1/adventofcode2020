...
for (var i = 0, iMax = passeports.length; i < iMax; ++i) {
	...
	for (var j = 0, jMax = champs.length; j < jMax; ++j) {
		var champ = champs[j].split(":");
		var pos = must.indexOf(champ[0]);
		if (pos != -1) {
			var validation = false;
			switch (champ[0]) {
				case "byr":
					var val = parseInt(champ[1], 10);
				...
				case "eyr":
					...
					break;
				case "hgt":
					var checkCM = champ[1].match(/^(\d+)cm$/);
					var checkIN = champ[1].match(/^(\d+)in$/);
					if (checkCM) {
						var val = parseInt(checkCM[1], 10);
						if ((val >= 150) && (val <= 193)) {
							validation = true;
						}
					}
					else if (checkIN) {
						var val = parseInt(checkIN[1], 10);
						if ((val >= 59) && (val <= 76)) {
							validation = true;
						}
					}
					break;
				case "hcl":
					if(champ[1].match(/^#[0-9a-f]{6}$/)) {
						validation = true;
					}
					break;
				case "ecl":
					if(champ[1].match(/^(amb|blu|brn|gry|grn|hzl|oth)$/)) {
						validation = true;
					}
					break;
				case "pid":
					if(champ[1].match(/^\d{9}$/)) {
						validation = true;
					}
					break;
			}
