...
for (var i = 0, iMax = input.length; i < iMax-2; ++i) {
	for (var j = i+1; j < iMax-1; ++j) {
		for (var k = j+1; k < iMax; ++k) {
			if (input[i] + input[j] + input[k] == 2020) {
				console.log("input[i] = " + input[i]);
				console.log("input[j] = " + input[j]);
				console.log("input[k] = " + input[k]);
				console.log("Produit = " + (input[i]*input[j]*input[k]));
			}
		}
	}
}
