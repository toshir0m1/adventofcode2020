...
do {
	...
	switch (command) {
		...
		case "acc":
			...
			break;
		case "jmp":
			next += parseInt(arg, 10);
			break;
	}
}
while (mem.indexOf(next) == -1);
...
for (var i = 0, iMax = program.length; i < iMax; ++i) {
	var sortie = false;
	var tempProgram = program.cloner();
	accumulator = 0, cursor = 0, next = 0, mem = [];
	
	var checkNopJmp = tempProgram[i].match(/^(nop|jmp)/);
	if (!checkNopJmp) {
		continue;
	}
	if (checkNopJmp[1] == "nop") {
		tempProgram[i] = tempProgram[i].replace("nop", "jmp");
	}
	else {
		tempProgram[i] = tempProgram[i].replace("jmp", "nop");
	}
	...
	do {
		...
		var line = tempProgram[cursor].split(" ");
		var command = line[0];
		var arg = line[1];
	...
	while ((mem.indexOf(next) == -1) && (!sortie));
}
