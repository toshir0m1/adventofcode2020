...
for (var com of commands) {
	var c = com[0];
	var val = +com.substr(1);
	
	switch (c) {
		case "N": position[1] += val; break;
		case "S": position[1] -= val; break;
		case "E": position[0] += val; break;
		case "W": position[0] -= val; break;
		case "L":
			for (var t = 0, turns = (val / 90); t < turns; t++) {
				switch (facing) {
...
console.log("Part 1 : " + (Math.abs(position[0]) + Math.abs(position[1])));
...
for (var com of commands) {
	...
	switch (c) {
		case "N": waypoint[1] += val; break;
		case "S": waypoint[1] -= val; break;
		case "E": waypoint[0] += val; break;
		case "W": waypoint[0] -= val; break;
		case "L":
			for (var t = 0, turns = (val / 90); t < turns; t++) {
...
console.log("Part 2 : " + (Math.abs(position[0]) + Math.abs(position[1])));
